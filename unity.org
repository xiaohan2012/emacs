...
#+begin_src emacs-lisp
  ...
  (defun unity/gsutil-ls-datagen-output-bucket ()
    (interactive)
    (let* (
      ...
      (gsutil/ls-nint (format "%s/%s"  output-dir name))
      )
    )
...
#+RESULTS:
: unity/gsutil-ls-datagen-output-bucket
