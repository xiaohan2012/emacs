...
#+begin_src emacs-lisp
  (defun unity/gls-training-data-at-point ()
    ...
    (let* (
           ...
           (root-uri
            (let* (
              ...
              (if (> (if dev-pos dev-pos 0) prd-pos)
                  "unity-ads-dd-ds-dev-prd-training-data"
                "unity-ads-dd-ds-prd-training-data"
                )
              )          
            )
           )
      (unity/gls-uri (format "gs://%s/cpi_conversion/%s" root-uri data-version))
      )
    )
#+end_src

#+RESULTS:
: unity/gls-training-data-at-point
