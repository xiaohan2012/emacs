...
#+begin_src emacs-lisp
  ...
  (defun unity/get-kubeflow-upload-command (pipeline-name ds-env)
    "get the command to upload a pipeline"  
    (format
     "python kubeflow/pipelines/conv/%s.py --ds_env %s  --do_upload" pipeline-name ds-env))
        ...
        (dolist (traffic-type traffic-types)
          (dolist (platform platforms)
            (let* ((run-name (format run-name-template git-branch traffic-type platform))
              ...
              (unity/vm-shell/send-string "sleep 1")              
              (unity/vm-shell/send-string (format "%s %s" command "--fetch_enrichment_data"))
              ;; (print command)
              )))
        )
      (unity/vm-shell/exit-kubeflow-shell)
...
** s2i upload pipelines


#+begin_src emacs-lisp
  (defun unity/upload-cpi-pipelines ()
    "submit one or more CPI train jobs for some git branch"
    (interactive)
    (let*
        (
         (git-branches (completing-read-multiple "Git branches: " (magit-list-branch-names)))
         (ds-env (completing-read "ds_env: " (list dev-ds-env prd-ds-env) nil t prd-ds-env nil prd-ds-env))
         (traffic-types (completing-read-multiple "Traffic types: " '("advctx" "bhv")))
         (platforms (completing-read-multiple "Platforms: " '("android" "ios")))
         )
      (unity/vm-shell/ensure-ssh-terminal-is-visible)
      ...
      (dolist (git-branch git-branches)
        ...
        (dolist (traffic-type traffic-types)
          (dolist (platform platforms)
            (let* ((datagen-command (unity/get-kubeflow-upload-command (format "cpi_%s_datagen_%s" traffic-type platform) ds-env))
                   (train-command (unity/get-kubeflow-upload-command (format "cpi_%s_train_%s" traffic-type platform) ds-env))
                   (serving-command (unity/get-kubeflow-upload-command (format "cpi_%s_serving_model_only_%s" traffic-type platform) ds-env))
                   )
              ;; (print command)
              ;; (unity/vm-shell/send-string "sleep 1") ; to avoid trained models with the same time
              ;; (print datagen-command)
              ;; (print train-command)
              ;; (print serving-command)
              (unity/vm-shell/send-string datagen-command)
              (unity/vm-shell/send-string train-command)
              (unity/vm-shell/send-string serving-command)
              ;; (print (format "%s-%d" command i))
              )))
        ;; remember to upload the trigger_full pipeline
        (unity/vm-shell/send-string "python kubeflow/pipelines/common/trigger_full.py --ds_env prd  --do_upload")
        )
      (unity/vm-shell/exit-kubeflow-shell)
      ))
...
** copy my GCS dev bucket URI

#+begin_src emacs-lisp
  (defun unity/copy-my-dev-bucket ()
    "copy GCS dev bucket URI"
    (interactive)
    (kill-new "gs://han-xiao-dev")
    )
#+end_src

#+RESULTS:
: unity/copy-my-dev-bucket
