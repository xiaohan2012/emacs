...
*** backup
#+begin_src emacs-lisp
  (defun unity/submit-pipeline-k-times (model-type ds-env exp-name run-name reps)
    "submit a pipeline job to KF"  
    (interactive (list (completing-read "ds_env: " (list dev-ds-env prd-ds-env) nil t dev-ds-env nil dev-ds-env)
		       (read-string "experiment name: " "han/exps")
		       (read-string "run name: " (format "s2c-train-%s-test" model-type))
		       (read-number "number of repetitions: " 1)
		       ))
    (let* ((command (unity/get-train-command (format "s2c_%s_train" model-type) ds-env exp-name run-name)))
...
#+begin_src emacs-lisp
  (defun unity/submit-s2c-train-jobs ()
    ...
    (interactive)
    (let* ((model-type (completing-read "model type: " '("bhv" "advctx" "ctx")))
	   ...
	   (command (unity/get-train-command (format "s2c_%s_train" model-type) ds-env exp-name run-name))
	   )
      ;; show the ssh buffer in another buffer
      (split-window-right)
      (other-window 1)
      (switch-to-buffer (unity/vm-shell/get-buffer))
      (other-window 1)
