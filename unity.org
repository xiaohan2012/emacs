...
- [ ] "submit" command in org-babel and execute it on VM  
...
#+begin_src emacs-lisp
  ...
  (defun unity/get-train-command (pipeline-name ds-env exp-name run-name)
    "get the command to run a training pipeline"  
    (format
...
- possible improvement:
  - ds_env choose from two options
  - reduce the amount of boiler plate code
    
#+begin_src emacs-lisp
  (defun unity/train-s2c-bhv (ds-env exp-name run-name)
    "submit a s2c train bhv job to KF"
    (interactive (list (read-string "ds_env: " dev-ds-env)
		       (read-string "experiment name: " "han/exps")
		       (read-string "run name: " "s2c-train-bhv-test")))
    (let* ((command (unity/get-train-command "s2c_bhv_train" ds-env exp-name run-name)))
  ...
  (defun unity/train-s2c-advctx (ds-env exp-name run-name)
    "submit a s2c train bhv job to KF"
    (interactive (list (read-string "ds_env: " dev-ds-env)
		       (read-string "experiment name: " "han/exps")
		       (read-string "run name: " "s2c-train-advctx-test")))
    (let* ((command (unity/get-train-command "s2c_advctx_train" ds-env exp-name run-name)))
      (unity/vm-shell/enter-kubeflow-shell)
      (unity/vm-shell/send-string command)
      (unity/vm-shell/exit-kubeflow-shell)
      )
  ...
  (defun unity/train-s2c-ctx (ds-env exp-name run-name)
    "submit a s2c train ctx job to KF"
    (interactive (list (read-string "ds_env: " dev-ds-env)
		       (read-string "experiment name: " "han/exps")
		       (read-string "run name: " "s2c-train-ctx-test")))
    (let* ((command (unity/get-train-command "s2c_ctx_train" ds-env exp-name run-name)))
      (unity/vm-shell/enter-kubeflow-shell)
      (unity/vm-shell/send-string command)
      (unity/vm-shell/exit-kubeflow-shell)
      )
...
#+begin_src emacs-lisp
  (defvar unity/vm-hostname "hanxiao-dev-vm" "hostname of my current dev VM")
  ...
  (defun ensure-ssh-terminal (hostname)
    ...
    (let* ((ssh-buffer-name (get-ssh-buffer-name hostname)))
	  ...
	  (ssh hostname)
	;; (message (format "'%s' is opened already in %s" hostname ssh-buffer-name)
	  )
	)
      ssh-buffer-name
      )
  ...
  (defun unity/vm-shell//ensure-ssh-terminal ()
    ...
    (interactive)
    (ensure-ssh-terminal unity/vm-hostname)
    )
#+end_src
...
#+begin_src emacs-lisp
  ...
  (no need to append a newline)"
    (comint-send-string
     (get-ssh-buffer-name my/vm-hostname)
     (format "%s\n" string))
    )
...
#+begin_src emacs-lisp
  (defun unity/gsutil-ls-path (path)
    (my/async-shell-command (format "gsutil ls %s" path) "*unity/gsutil/ls-results*")
    )
