...
(defun filter-osc-9-4-compile ()
  "Filter out OSC 9;4 escape sequences in compile buffer."
  (save-excursion
    (goto-char compilation-filter-start)
    ;; Remove OSC 9;4 sequences with surrounding characters
    (while (re-search-forward "[^\n]*\\]9;4;[0-9];[0-9]*\\\\[^\n]*" nil t)
      (replace-match ""))
    ;; Clean up standalone backslashes at start of lines
    (goto-char compilation-filter-start) 
    (while (re-search-forward "^\\\\=" nil t)
      (replace-match "="))
    ;; Clean up backslashes around dots (test progress indicators)
    (goto-char compilation-filter-start)
    (while (re-search-forward "\\\\\\." nil t)
      (replace-match "."))
    ;; Clean up trailing backslashes after percentages
    (goto-char compilation-filter-start)
    (while (re-search-forward "\\]\\\\$" nil t)
      (replace-match "]"))))
...
#+RESULTS:
| debug-filter-osc-9-4-compile | filter-osc-9-4-compile | filter-compile-buffer-escape-sequences | #[nil ((save-excursion (goto-char compilation-filter-start) (while (re-search-forward \]9;[0-9;]+\\ nil t) (replace-match )))) nil] |
